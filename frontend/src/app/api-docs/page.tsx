...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    title: "Admin (Super)",
    icon: Shield,
    description: "Painel de super-admin. Requer role=super_admin. Gerencia usuários, planos, config de pagamento, proxy global e auditoria.",
    badge: "Super-Admin",
    endpoints: [
      { method: "GET",    path: "/v1/admin/stats", description: "KPIs globais da plataforma (usuários ativos, MRR, churn)", auth: "bearer" },
      { method: "GET",    path: "/v1/admin/users", description: "Listar TODOS os usuários (paginado, com filtros)", auth: "bearer", params: { limit: "50", role: "user", is_blocked: "false" } },
      { method: "POST",   path: "/v1/admin/users", description: "Criar usuário manualmente (bypass signup)", auth: "bearer", body: { name: "...", email: "...", password: "...", role: "user" } },
      { method: "PUT",    path: "/v1/admin/users/{id}", description: "Atualizar qualquer dado do usuário (role, plan, bloqueio, saldo)", auth: "bearer" },
      { method: "DELETE", path: "/v1/admin/users/{id}", description: "Remover usuário (soft-delete)", auth: "bearer" },
      { method: "POST",   path: "/v1/admin/users/{id}/reset-password", description: "Redefinir senha de qualquer usuário", auth: "bearer", body: { new_password: "..." } },
      { method: "GET",    path: "/v1/admin/plans", description: "Listar planos configurados no sistema", auth: "bearer" },
      { method: "POST",   path: "/v1/admin/plans", description: "Criar plano customizado", auth: "bearer", body: { name: "Enterprise", price_brl: 999, limits: { instances: 50 } } },
      { method: "PUT",    path: "/v1/admin/plans/{id}", description: "Atualizar plano", auth: "bearer" },
      { method: "GET",    path: "/v1/admin/payment-settings", description: "Configurações dos provedores de pagamento (stripe/asaas keys, webhook secrets)", auth: "bearer" },
      { method: "PUT",    path: "/v1/admin/payment-settings", description: "Atualizar config de pagamento", auth: "bearer", body: { stripe_secret_key: "sk_live_...", asaas_api_key: "..." } },
      { method: "GET",    path: "/v1/admin/proxy-config", description: "Listar config global de proxy", auth: "bearer" },
      { method: "PUT",    path: "/v1/admin/proxy-config", description: "Atualizar config global de proxy", auth: "bearer" },
      { method: "DELETE", path: "/v1/admin/proxy-config/{id}", description: "Remover entrada de proxy config", auth: "bearer" },
      { method: "POST",   path: "/v1/admin/proxy-test", description: "Testar proxy global", auth: "bearer" },
      { method: "GET",    path: "/v1/admin/proxy-stats", description: "Estatísticas de uso dos proxies (quais estão saturados)", auth: "bearer" },
      { method: "GET",    path: "/v1/admin/invites", description: "Listar convites (toggle invite-only)", auth: "bearer" },
      { method: "POST",   path: "/v1/admin/invites/toggle", description: "Ativar/desativar invite-only no cadastro público", auth: "bearer", body: { enabled: true } },
      { method: "GET",    path: "/v1/admin/inspect/servers", description: "Inspeção: todos os servidores com saúde/instâncias", auth: "bearer" },
      { method: "GET",    path: "/v1/admin/inspect/instances", description: "Inspeção: todas as instâncias do sistema (com JID, status, owner)", auth: "bearer" },
      { method: "GET",    path: "/v1/admin/inspect/instances/{id}", description: "Inspeção profunda de uma instância específica", auth: "bearer" },
    ],
  },
  {
    id: "realtime",
