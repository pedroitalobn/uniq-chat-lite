...
import {
  ...
  Key, FileJson, ExternalLink, Loader2, Shield, Link2, Copy,
} from "lucide-react";
import { integrationsApi, globalWebhooksApi, apiKeysApi, proxyPoolsApi } from "@/lib/api";
...
function WebhooksSection() {
  ...
  const create = useMutation({ mutationFn: (url: string) => globalWebhooksApi.create({ name: "Webhook", url, events: [] }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["global-webhooks"] }); toast.success("Webhook criado"); } });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl p-5 space-y-4" style={{ background: "hsl(240 18% 6%)", border: "1px solid hsl(240 12% 13%)" }}>
        ...
        <p className="text-xs" style={{ color: "hsl(240 8% 46%)" }}>Receba eventos de todas as instâncias (mensagens recebidas/enviadas, status, etc.)</p>
        <div className="flex gap-3">
          <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => e.key === "Enter" && url && create.mutate(url)}
            placeholder="https://seu-webhook.com.br/webhook" className="input-field flex-1" />
          <button onClick={() => create.mutate(url)} disabled={!url || create.isPending} className="btn-primary flex items-center gap-2 text-sm px-4 py-2.5 disabled:opacity-40">
            {create.isPending ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />} Criar
          </button>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div>{data.map((w: any, i: number) => (
            <div key={w.id} className="px-5 py-4 flex items-center gap-4 transition-colors" style={{ borderBottom: i < data.length - 1 ? "1px solid rgba(255,255,255,0.04)" : undefined }}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: "hsl(240 15% 80%)" }}>{w.url}</p>
                <p className="text-xs mt-0.5" style={{ color: "hsl(240 8% 38%)" }}>Eventos: {w.events?.join(", ") || "todos"}</p>
              </div>
      ...
      <div className="rounded-2xl p-5 space-y-3" style={{ background: "hsl(240 18% 6%)", border: "1px solid hsl(240 12% 13%)" }}>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs" style={{ color: "hsl(240 8% 55%)" }}>
          <div><code style={{ color: "var(--green)" }}>message.received</code> - Nova mensagem recebida</div>
          <div><code style={{ color: "var(--green)" }}>message.sent</code> - Mensagem enviada</div>
          <div><code style={{ color: "var(--green)" }}>status.changed</code> - Status da instância</div>
          <div><code style={{ color: "var(--green)" }}>qr.updated</code> - QR Code atualizado</div>
          <div><code style={{ color: "var(--green)" }}>call.received</code> - Chamada recebida</div>
          <div><code style={{ color: "var(--green)" }}>group.joined</code> - Entrou em grupo</div>
        </div>
      </div>
    </div>
  );
...
const PROXY_PROVIDERS = [
  { id: "uniq", name: "Uniq Proxy", description: "Proxy nativo da Uniq.chat", color: "#00d46a" },
...
function ProxiesSection() {
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery<ProxyProviderConfig[]>({ queryKey: ["proxy-providers"], queryFn: () => proxyPoolsApi.listProviders().then(r => r.data) });
  const create = useMutation({ mutationFn: (d: any) => proxyPoolsApi.createProvider(d), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["proxy-providers"] }); toast.success("Proxy provider criado"); } });
  const del = useMutation({ mutationFn: (id: string) => proxyPoolsApi.deleteProvider(id), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["proxy-providers"] }); toast.success("Proxy provider removido"); } });
  const [showModal, setShowModal] = useState(false);
  const [selectedProvider, setSelectedProvider] = useState<any>(null);
  const [form, setForm] = useState({ name: "", api_key: "", country: "br" });

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {PROXY_PROVIDERS.map(p => (
          <button key={p.id} onClick={() => { setSelectedProvider(p); setShowModal(true); }} className="rounded-2xl border p-4 text-left transition-all hover:scale-[1.01]"
            style={{ background: `${p.color}08`, borderColor: `${p.color}30` }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3" style={{ background: `${p.color}18` }}><Link2 className="w-5 h-5" style={{ color: p.color }} /></div>
            <p className="text-sm font-semibold" style={{ color: "var(--text-1)" }}>{p.name}</p>
            <p className="text-xs mt-1" style={{ color: "var(--text-3)" }}>{p.description}</p>
          </button>
        ))}
      </div>
      {data && data.length > 0 && (
        <div className="space-y-2 mt-6">
          <h3 className="text-xs font-semibold uppercase tracking-widest" style={{ color: "var(--text-3)" }}>Proxies configurados</h3>
          {data.map(p => (
            <div key={p.id} className="rounded-2xl border p-4 flex items-center gap-4" style={{ background: "var(--surface-2)", borderColor: "var(--surface-border)" }}>
              <Globe className="w-5 h-5" style={{ color: "#06b6d4" }} />
              <div className="flex-1"><p className="text-sm font-semibold" style={{ color: "var(--text-1)" }}>{p.name}</p><p className="text-xs" style={{ color: "var(--text-3)" }}>{p.provider} · {p.api_key_masked} · {p.country}</p></div>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${p.is_active ? "text-green-400 bg-green-500/10" : "text-gray-400 bg-gray-500/10"}`}>{p.is_active ? "Ativo" : "Inativo"}</span>
              <button onClick={() => del.mutate(p.id)}><Trash2 className="w-4 h-4" style={{ color: "var(--text-3)" }} /></button>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="w-full max-w-md rounded-2xl border p-6 space-y-4" style={{ background: "var(--surface-2)", borderColor: "var(--surface-border)" }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${selectedProvider.color}18` }}><Link2 className="w-5 h-5" style={{ color: selectedProvider.color }} /></div>
              <h2 className="font-semibold" style={{ color: "var(--text-1)" }}>Configurar {selectedProvider.name}</h2>
            </div>
            <div><label className="text-xs font-medium block mb-1.5" style={{ color: "var(--text-2)" }}>Nome</label><input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="input-field w-full" placeholder="Meu Proxy Brasil" /></div>
            <div><label className="text-xs font-medium block mb-1.5" style={{ color: "var(--text-2)" }}>API Key</label><input value={form.api_key} onChange={e => setForm({...form, api_key: e.target.value})} type="password" className="input-field w-full" placeholder="sk-..." /></div>
            <div><label className="text-xs font-medium block mb-1.5" style={{ color: "var(--text-2)" }}>País padrão</label><select value={form.country} onChange={e => setForm({...form, country: e.target.value})} className="input-field w-full"><option value="br">Brasil</option><option value="us">Estados Unidos</option><option value="global">Global</option></select></div>
            <div className="flex gap-2 pt-2"><button onClick={() => setShowModal(false)} className="btn-ghost flex-1">Cancelar</button><button onClick={() => { create.mutate({ provider: selectedProvider.id, name: form.name, api_key: form.api_key, country: form.country }); setShowModal(false); setForm({ name: "", api_key: "", country: "br" }); }} disabled={!form.name || !form.api_key} className="btn-primary flex-1">Salvar</button></div>
          </div>
        </div>
      )}
    </div>
