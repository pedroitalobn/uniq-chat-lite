...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      { method: "POST", path: "/instances/{id}/messages/buttons",      summary: "Enviar botões (reply/url/call)", pathParams: ["id"],
        body: {
          to:      { type: "string", required: true, description: "Número destinatário",      example: "5511999999999" },
          ...
          buttons: { type: "array",  required: true, description: "Lista de botões",           example: '[{"id":"sim","text":"Sim","type":"reply"},{"text":"Abrir site","type":"url","url":"https://uniq.chat"}]' },
        } },
      { method: "POST", path: "/instances/{id}/messages/template",     summary: "Enviar template message (quick reply/url/call)", pathParams: ["id"],
        body: {
          to:      { type: "string", required: true, description: "Número destinatário",       example: "5511999999999" },
          ...
          buttons: { type: "array",  required: true, description: "Array de {display_text, type, id|url|phone_number}", example: '[{"display_text":"Sim","type":"quickreply","id":"yes"}]' },
        } },
      { method: "POST", path: "/instances/{id}/messages/list",         summary: "Enviar lista interativa",     pathParams: ["id"],
        body: {
          to:          { type: "string", required: true, description: "Número destinatário",  example: "5511999999999" },
          ...
          footer:      { type: "string", required: false,description: "Rodapé" },
          sections:    { type: "array",  required: true, description: "Seções com rows",      example: '[{"title":"Menu","rows":[{"id":"1","title":"Item 1"}]}]' },
        } },
      { method: "POST", path: "/instances/{id}/messages/menu",         summary: "Menu unificado (button/list/poll/carousel)", pathParams: ["id"],
        body: {
  ...
  },
  {
    ...
    icon: <ShieldCheck className="w-3.5 h-3.5" />,
    endpoints: [
      { method: "GET",    path: "/admin/stats",                 summary: "KPIs globais da plataforma" },
      { method: "GET",    path: "/admin/users",                 summary: "Listar todos os usuários (paginado)" },
      { method: "POST",   path: "/admin/users",                 summary: "Criar usuário (bypass signup)",
        body: {
          name:     { type: "string", required: true, description: "Nome" },
          email:    { type: "string", required: true, description: "E-mail" },
          password: { type: "string", required: true, description: "Senha" },
          role:     { type: "string", required: false, description: "user | admin | super_admin" },
        } },
      { method: "PUT",    path: "/admin/users/{id}",            summary: "Atualizar qualquer dado do usuário", pathParams: ["id"] },
      { method: "DELETE", path: "/admin/users/{id}",            summary: "Remover usuário (soft-delete)", pathParams: ["id"] },
      { method: "POST",   path: "/admin/users/{id}/reset-password", summary: "Redefinir senha de qualquer usuário", pathParams: ["id"],
        body: { new_password: { type: "string", required: true, description: "Nova senha" } } },
      { method: "GET",    path: "/admin/plans",                 summary: "Listar planos configurados" },
      { method: "POST",   path: "/admin/plans",                 summary: "Criar plano customizado" },
      { method: "PUT",    path: "/admin/plans/{id}",            summary: "Atualizar plano", pathParams: ["id"] },
      { method: "GET",    path: "/admin/payment-settings",      summary: "Config dos provedores de pagamento" },
      { method: "PUT",    path: "/admin/payment-settings",      summary: "Atualizar config de pagamento" },
      { method: "GET",    path: "/admin/proxy-config",          summary: "Config global de proxy" },
      { method: "PUT",    path: "/admin/proxy-config",          summary: "Atualizar proxy global" },
      { method: "DELETE", path: "/admin/proxy-config/{id}",     summary: "Remover proxy config", pathParams: ["id"] },
      { method: "POST",   path: "/admin/proxy-test",            summary: "Testar proxy global" },
      { method: "GET",    path: "/admin/proxy-stats",           summary: "Estatísticas de uso dos proxies" },
      { method: "GET",    path: "/admin/invites",               summary: "Listar convites (toggle invite-only)" },
      { method: "POST",   path: "/admin/invites/toggle",        summary: "Ativar/desativar invite-only",
        body: { enabled: { type: "boolean", required: true, description: "Ligado/desligado" } } },
      { method: "GET",    path: "/admin/inspect/servers",       summary: "Todos os servidores com saúde/instâncias" },
      { method: "GET",    path: "/admin/inspect/instances",     summary: "Todas as instâncias (JID, status, owner)" },
      { method: "GET",    path: "/admin/inspect/instances/{id}", summary: "Inspeção profunda de uma instância", pathParams: ["id"] },
    ],
  },
  {
    id: "permissions",
    ...
    endpoints: [
      ...
      { method: "POST", path: "/permissions/seed", summary: "Seed inicial de permissões padrão (admin)" },
    ],
  },
];
